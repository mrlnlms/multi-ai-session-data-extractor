...
## 13. Contratos de captura complementares

- Em conversas compartilhadas, `id` e o UUID publico da URL; use
  `conversation_id` para buscar a conversa canonica. Tratar os dois como o
  mesmo identificador produz 404 no endpoint de conversa.
- O endpoint de download de arquivo devolve uma URL preassinada. Para arquivos
  de projeto, o `gizmo_id` precisa acompanhar o pedido; sem esse contexto o
  servidor retorna erro de permissao.
- `message.content.parts` mistura strings e objetos. Transcricoes de voz vem
  em objetos `audio_transcription`, para entrada e saida; um parser que itera
  apenas strings perde esse conteudo.
- Arquivos e imagens antigos podem ter sido removidos upstream. Registre a
  falha no manifest e preserve a referencia, em vez de rebaixar a conversa ou
  repetir indefinidamente o download.
