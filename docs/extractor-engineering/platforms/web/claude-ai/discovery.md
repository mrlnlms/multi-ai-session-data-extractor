...
via `scripts/claude-sync.py`). Espelha
...
## 9. Contratos de captura complementares

- A API autenticada preserva campos que podem ser omitidos do export oficial;
  a captura normal deve usar a API, nao o export como fonte de completude.
- Conversas, projetos, documentos de projeto, memoria e arquivos usam rotas
  distintas da organizacao. Nunca incluir a rota de perfil na captura: ela
  pode expor dados de cobranca sensiveis.
- `file_kind` determina a variante de download: imagens usam `preview`,
  documentos podem expor apenas thumbnail e blobs de texto ja possuem
  conteudo inline. Nao trate todos como binario recuperavel.
- O `org_id` vem da sessao autenticada. Contexto Playwright persistente e os
  cookies Cloudflare sao o contrato de transporte; uma sessao expirada exige
  novo login, nao um bypass alternativo.

Cobertura atual, CRUD validado e limites remanescentes ficam no `state.md` e
em [known-limitations.md](../../../known-limitations.md). Crie um
`server-behavior.md` apenas quando houver um conjunto independente de fatos
upstream que nao caiba no estado atual.
