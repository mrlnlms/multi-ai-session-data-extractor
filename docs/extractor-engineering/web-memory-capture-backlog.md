...
| [Kimi](platforms/web/kimi/state.md): Memory Instructions, Dream Memory, account instructions and actual Projects | The observed Memory Instructions collection and global instruction field were empty; Dream Memory was disabled. The sample Project had empty Instructions/Files, and no independent Project-memory collection was shown. Current sync does not capture these objects. | Identify the native read transports and preserve even a verified empty response with account/Project scope. Validate non-empty records when they naturally exist or the owner explicitly chooses a controlled sample; keep Saved Prompts outside native memory. |
