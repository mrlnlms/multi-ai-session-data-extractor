...
Perplexity account Memory, Qwen account Memory plus Customize Qwen, and Gemini
account Instructions are already captured and projected. Project
...
| [Gemini](platforms/web/gemini/state.md): conversation-derived Memory | Account-global Instructions have cumulative native raw capture and canonical projection. The enabled learned-Memory control did not expose a readable item list or stable native envelope in the bounded probe. | Reopen learned-Memory capture only if an inspectable native record/payload becomes available; do not reconstruct it from answers. |
