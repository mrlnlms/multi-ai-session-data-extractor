...
| Web memory/configuration capability census | **Active; ChatGPT, Claude.ai and Perplexity memory capture published 2026-09-25; Kimi raw read added locally** | [Pending capture](extractor-engineering/web-memory-capture-backlog.md), [memory architecture](product/agent-memory-architecture.md) | Published memory Parquets cover the first three web sources. Kimi's native Memory/settings/Dream/Project reads have no canonical memory projection or DVC publication yet; follow the remaining evidence gates. |
