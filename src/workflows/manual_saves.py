"""Import manual saves and materialize them by destination platform.
...
def run_manual_saves(
    external_dir: Path,
    processed_dir: Path,
) -> dict[str, object]:
    """Run every available manual importer and write platform Parquets.

    Paths are explicit so callers and tests do not depend on repository-global
    state. The returned summary is also the payload persisted in the capture
    log by :func:`main`.
    """
    ...
    p = external_dir / "clippings-obsidian"
    if p.exists():
    ...
    p = external_dir / "terminal-claude-code"
    if p.exists():
...
def append_capture_log(external_dir: Path, summary: dict[str, object]) -> Path:
    """Append a completed manual-import summary to its operational log."""
    external_dir.mkdir(parents=True, exist_ok=True)
    log_path = external_dir / "capture_log.jsonl"
