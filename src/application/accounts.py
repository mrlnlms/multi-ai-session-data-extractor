...
from src.platforms.registry import PLATFORM_ACCOUNT_CAPABILITIES
from src.workflows.account_sync import AccountSyncPlan, execute_account_sync, plan_account_sync
...
def account_actions(account: AccountState) -> tuple[AccountActionView, ...]:
    ...
    if capability:
        ...
        command = f"PYTHONPATH=. .venv/bin/python -m src.platforms.{source}.commands.login {capability.login_argument} {account.key}"
    return tuple(AccountActionView(name, account.account_id, PRESERVATION_NOTICE,
        ...
        for name in ("lifecycle", "bind", "auth-check", "sync", "login"))
