...
from src.platforms.chatgpt.extractor.models import ConversationMeta, ProjectMeta
    ...
    async def _get_token(self) -> str:
        ...
        if not response.ok:
            raise RuntimeError(
                ...
                "Rode 'PYTHONPATH=. .venv/bin/python -m src.platforms.chatgpt.commands.login'."
            )
        ...
        if not token:
            raise RuntimeError(
                ...
                "rode 'PYTHONPATH=. .venv/bin/python -m src.platforms.chatgpt.commands.login' de novo."
            )
        ...
        *,
        ...
        while True:
            ...
            if response.status in (401, 403):
                raise RuntimeError(
                    "Sessao ChatGPT expirou. Rode 'PYTHONPATH=. .venv/bin/python -m src.platforms.chatgpt.commands.login'."
                )
