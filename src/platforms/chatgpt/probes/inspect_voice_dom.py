...
Uso:
    PYTHONPATH=. .venv/bin/python -m src.platforms.chatgpt.probes.inspect_voice_dom <conv_id>

Requer login persistente via ``python -m src.platforms.chatgpt.commands.login`` antes.

Output:
    - tests/platforms/chatgpt/extractor/fixtures/voice_bubble.html (HTML da timeline)
...
FIXTURE_PATH = Path("tests/platforms/chatgpt/extractor/fixtures/voice_bubble.html")


async def inspect(conv_id: str):
    profile_dir = get_profile_dir()
    if not profile_dir.exists():
        print("ERRO: profile nao encontrado. Rode 'PYTHONPATH=. .venv/bin/python -m src.platforms.chatgpt.commands.login' primeiro.")
        sys.exit(1)
    ...
    async with async_playwright() as p:
        ...
        print("\nPopule SELECTORS em src/platforms/chatgpt/extractor/dom_voice.py com esses valores.\n")
