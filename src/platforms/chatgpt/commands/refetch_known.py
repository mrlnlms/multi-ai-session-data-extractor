...
Uso:
  PYTHONPATH=. .venv/bin/python -m src.platforms.chatgpt.commands.refetch_known [--account default]
                                                                 [--batch-size 10]
...
async def refetch(account: str, batch_size: int = DEFAULT_BATCH_SIZE) -> None:
    ...
    print("\nProximo passo:")
    print(f"  PYTHONPATH=. .venv/bin/python -m src.platforms.chatgpt.commands.reconcile {raw_dir}")
    print("  PYTHONPATH=. .venv/bin/python -m src.platforms.chatgpt.commands.parse")
