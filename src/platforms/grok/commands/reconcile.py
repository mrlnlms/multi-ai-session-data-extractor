...
Uso:
    PYTHONPATH=. .venv/bin/python -m src.platforms.grok.commands.reconcile            # raw=data/raw/Grok/
    PYTHONPATH=. .venv/bin/python -m src.platforms.grok.commands.reconcile --full
"""
...
from src.platforms.grok.extractor.orchestrator import BASE_DIR as RAW_DIR
from src.platforms.grok.reconciler import run_reconciliation
...
def main():
    ...
    if not raw.exists() or not (raw / "discovery_ids.json").exists():
        print(f"ERRO: raw nao encontrado em {raw}. Rode python -m src.platforms.grok.commands.export primeiro.")
        sys.exit(1)
    print(f"Raw: {raw}")
