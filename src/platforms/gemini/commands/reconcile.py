...
Uso:
    PYTHONPATH=. .venv/bin/python -m src.platforms.gemini.commands.reconcile --account 1 [raw_dir]
    PYTHONPATH=. .venv/bin/python -m src.platforms.gemini.commands.reconcile --account 1 --full
"""
...
def main():
    ...
    merged_base = Path("data/merged/Gemini") / f"account-{a.account}"
    prev = Path(a.previous_merged) if a.previous_merged else None

    r = run_reconciliation(raw, merged_base, prev, full=a.full)
    print("\n" + r.summary())
    if r.aborted:
