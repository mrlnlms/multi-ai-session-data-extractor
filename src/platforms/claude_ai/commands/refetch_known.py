...
Cenario tipico: ``python -m src.platforms.claude_ai.commands.sync`` terminou OK mas N convs falharam por
...
NAO eh state-refresh — pra isso, rode `python -m src.platforms.claude_ai.commands.sync --full`.

Uso:
    PYTHONPATH=. .venv/bin/python -m src.platforms.claude_ai.commands.refetch_known
    PYTHONPATH=. .venv/bin/python -m src.platforms.claude_ai.commands.refetch_known --profile default --retries 3
...
from src.platforms.claude_ai.extractor.orchestrator import BASE_DIR
...
async def main(args: argparse.Namespace) -> int:
    ...
    if not (raw_dir / "discovery_ids.json").exists():
        print(f"ERRO: discovery_ids.json nao existe em {raw_dir}. Rode python -m src.platforms.claude_ai.commands.sync primeiro.")
        return 1
    ...
    print("  PYTHONPATH=. .venv/bin/python -m src.platforms.claude_ai.commands.reconcile")
    print("  PYTHONPATH=. .venv/bin/python -m src.platforms.claude_ai.commands.parse")
