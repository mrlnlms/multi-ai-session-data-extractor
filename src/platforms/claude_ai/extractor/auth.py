...
Pattern espelha src/platforms/chatgpt/extractor/auth.py — launch_persistent_context mantem
...
async def login(profile_name: str = "default") -> None:
    ...
    print("Agora rode: PYTHONPATH=. .venv/bin/python -m src.platforms.claude_ai.commands.export")
...
async def load_context(profile_name: str = "default", headless: bool = True) -> tuple[BrowserContext, str]:
    ...
    if not profile_dir.exists():
        raise RuntimeError(
            f"Profile nao existe: {profile_dir}. Rode python -m src.platforms.claude_ai.commands.login primeiro."
        )
    ...
    if org_id_cache.exists():
        ...
        if not org_id:
            ...
            raise RuntimeError(
                ...
                "Faca login novamente: python -m src.platforms.claude_ai.commands.login"
            )
