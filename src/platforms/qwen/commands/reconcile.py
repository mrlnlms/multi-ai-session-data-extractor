...
Uso:
    PYTHONPATH=. .venv/bin/python -m src.platforms.qwen.commands.reconcile            # raw=data/raw/Qwen/
    PYTHONPATH=. .venv/bin/python -m src.platforms.qwen.commands.reconcile --full
"""
...
from src.platforms.qwen.extractor.orchestrator import BASE_DIR as RAW_DIR
from src.platforms.qwen.reconciler import run_reconciliation, FEATURE_FLAGS
...
def main():
    ...
    if not raw.exists() or not (raw / "discovery_ids.json").exists():
        print(f"ERRO: raw nao encontrado em {raw}. Rode python -m src.platforms.qwen.commands.export primeiro.")
        sys.exit(1)
    print(f"Raw: {raw}")
