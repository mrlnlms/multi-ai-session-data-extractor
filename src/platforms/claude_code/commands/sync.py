...
Idempotente. Uso: PYTHONPATH=. .venv/bin/python -m src.platforms.claude_code.commands.sync
"""
...
def main(
    *,
    ...
    runtime = load_asset_runtime("claude_code")
    ...
    ap.add_argument("--no-binaries", action="store_true",
                    help="(no-op: imagens inline pertencem ao JSONL e sao materializadas pelo parser)")
    ...
    parser.parse(
        RAW_DIR,
        home_memory_files=current_source_files("claude_code"),
    )
