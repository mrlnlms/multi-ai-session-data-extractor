"""Reconciler DeepSeek standalone (pasta unica)."""

import argparse
import sys
from pathlib import Path

from src.platforms.deepseek.extractor.orchestrator import BASE_DIR as RAW_DIR
from src.platforms.deepseek.reconciler import run_reconciliation, FEATURE_FLAGS
...
def main():
    ...
    if not raw.exists() or not (raw / "discovery_ids.json").exists():
        print(f"ERRO: raw nao encontrado em {raw}. Rode python -m src.platforms.deepseek.commands.export primeiro.")
        sys.exit(1)
    print(f"Raw: {raw}")
