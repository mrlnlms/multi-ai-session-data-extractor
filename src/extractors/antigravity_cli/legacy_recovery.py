...
def source_pb(conversation_id: str, source_root: Path, raw_root: Path) -> Path:
    ...
    if not raw.exists():
        raise RuntimeError(f"{conversation_id}: raw PB absent; execute scripts/platform/antigravity-cli/sync.py first")
    if not source.exists():
        raise RuntimeError(f"{conversation_id}: source PB absent from {source_root}")
    if sha256(source) != sha256(raw):
        raise RuntimeError(f"{conversation_id}: raw PB differs from local source; execute scripts/platform/antigravity-cli/sync.py first")
    return raw
