"""Reconciler do Qwen — merge raws preservando historico.

Layout: data/raw/Qwen Data/<YYYY-MM-DDTHH-MM>/
  - conversations/<id>.json
  - projects.json
  - discovery_ids.json: [{id, title, updated_at, created_at, ...}]
  - assets/

Schema discovery: id, updated_at (epoch float), pinned, project_id, chat_type.

Saida: data/merged/Qwen/<YYYY-MM-DD>/
"""
...
class QwenReconcileReport:
    ...
    def summary(self) -> str:
        return (
            f"Reconciliacao Qwen: added={self.added}, updated={self.updated}, "
...
def run_reconciliation(
    ...
    if prev and len(curr) / max(len(prev), 1) < DROP_THRESHOLD:
        return QwenReconcileReport(aborted=True,
    ...
    def _write_with_seen(src: Path, dst: Path):
        try:
    ...
    for uid in plan.to_use:
        ...
        if s.exists(): _write_with_seen(s, d)
    ...
    # projects.json: copia do raw atual sempre
    pj_src = raw_dir / "projects.json"
    if pj_src.exists():
        shutil.copy2(pj_src, out / "projects.json")
