...
def test_gemini_cli_branches_one_per_conv(tmp_path):
    ...
    assert b.source == "gemini_cli"


def test_gemini_cli_write_parquets(tmp_path):
    raw = tmp_path / "raw"
    raw.mkdir()
    ...
    expected = {
        "gemini_cli_conversations.parquet",
