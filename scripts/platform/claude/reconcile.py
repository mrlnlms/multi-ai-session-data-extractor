...
Uso:
    PYTHONPATH=. .venv/bin/python scripts/platform/claude/reconcile.py            # raw=data/raw/Claude.ai/, merged=data/merged/Claude.ai/
    PYTHONPATH=. .venv/bin/python scripts/platform/claude/reconcile.py --full
    PYTHONPATH=. .venv/bin/python scripts/platform/claude/reconcile.py --refetch-features attachments_extracted_content

Default: pasta unica cumulativa. scripts/platform/claude/sync.py ja chama internamente.
"""
...
def main():
    ...
    if not raw.exists() or not (raw / "discovery_ids.json").exists():
        print(f"ERRO: raw nao encontrado em {raw}. Rode scripts/platform/claude/export.py primeiro.")
        sys.exit(1)
    print(f"Raw: {raw}")
