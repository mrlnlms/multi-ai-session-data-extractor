...
Cenario tipico: scripts/platform/claude/sync.py terminou OK mas N convs falharam por
...
NAO eh state-refresh — pra isso, rode `scripts/platform/claude/sync.py --full`.

Uso:
    PYTHONPATH=. .venv/bin/python scripts/platform/claude/refetch-known.py
    PYTHONPATH=. .venv/bin/python scripts/platform/claude/refetch-known.py --profile default --retries 3
...
async def main(args: argparse.Namespace) -> int:
    ...
    if not (raw_dir / "discovery_ids.json").exists():
        print(f"ERRO: discovery_ids.json nao existe em {raw_dir}. Rode scripts/platform/claude/sync.py primeiro.")
        return 1
    ...
    print("  PYTHONPATH=. .venv/bin/python scripts/platform/claude/reconcile.py")
    print("  PYTHONPATH=. .venv/bin/python scripts/platform/claude/parse.py")
