...
Uso:
    PYTHONPATH=. .venv/bin/python scripts/platform/chatgpt/probes/inspect-voice-dom.py <conv_id>

Requer login persistente via scripts/platform/chatgpt/login.py antes.
...
async def inspect(conv_id: str):
    ...
    if not profile_dir.exists():
        print("ERRO: profile nao encontrado. Rode 'PYTHONPATH=. .venv/bin/python scripts/platform/chatgpt/login.py' primeiro.")
        sys.exit(1)
