...
#
# Uso:
#   scripts/workflows/serve-qmds.sh           # comportamento default = start
#   scripts/workflows/serve-qmds.sh start     # sobe servidor + abre browser
#   scripts/workflows/serve-qmds.sh stop      # mata servidor
#   scripts/workflows/serve-qmds.sh restart   # stop + start
#   scripts/workflows/serve-qmds.sh status    # ta rodando ou nao?
#   scripts/workflows/serve-qmds.sh open      # abre todas as 15 abas no browser
#
# Variaveis (opcionais):
...
cmd_start() {
  ...
  if lsof -ti:"$PORT" >/dev/null 2>&1; then
    ...
    echo "  rode com PORT=8766 scripts/workflows/serve-qmds.sh, ou identifique:" >&2
    echo "  lsof -i:$PORT" >&2
    return 1
  fi
